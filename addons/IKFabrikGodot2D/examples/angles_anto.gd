...
func angle_in_range():#ANTO
	if a0 < a1:
		...
		return ((a <= a1 and a >= -PI) or (a <= PI and a >= a0))##ANTO

func closer_angle():#ANTO
	######## this part can be shortened for sure, but it works now, so i didn't want to attempt it #############
	var _a  = a + PI
	var _a0 = a0 + PI
	var _a1 = a1 + PI
...
func _draw():
	...
	if not ((a0 < a1 and a0+a_width>= -PI) or (a1<a0 and a0+a_width>= PI)): #ANTO
...
func calculate_range_points():
	var iter = 50
	var _sign = 1 if ((a0 < a1 and a0+a_width>= -PI) or (a1<a0 and a0+a_width>= PI)) else -1 #ANTO
	var increment = _sign * abs(a_width)/iter #ANTO
	var points = []
